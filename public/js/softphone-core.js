    ...
    class SokratSoftphoneCore {
        ...
        async connect(preset, secret) {
            ...
            try {
                ...
                let wssUrl = defaultWss;
                if (preset.wssUrl) {
                    try {
                        const parsed = new URL(preset.wssUrl);
                        if (parsed.hostname === host || parsed.hostname === '127.0.0.1' || parsed.hostname === 'localhost' || parsed.port === '8089') {
                            wssUrl = defaultWss;
                        } else {
                            wssUrl = preset.wssUrl;
                        }
                    } catch (_) {
                        wssUrl = defaultWss;
                    }
